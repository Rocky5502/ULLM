...
def main() -> None:
    ...
    p.add_argument("--summary", type=Path, default=Path("results/processed/summary_neutral.csv"))
    p.add_argument("--sampling", type=Path, default=Path("results/processed/sampling.csv"))
    p.add_argument("--ranking", type=Path, default=Path("results/processed/sampling_ranking.csv"))
    p.add_argument("--selective", type=Path, default=Path("results/processed/selective.csv"))
    p.add_argument("--pairwise", type=Path, default=Path("results/processed/pairwise.csv"))
    p.add_argument("--robustness", type=Path, default=Path("results/processed/prompt_robustness.csv"))
    p.add_argument("--recheck", type=Path, default=Path("results/processed/recheck.csv"))
    p.add_argument("--outdir", type=Path, default=Path("results/figures"))
    args = p.parse_args()

    # RQ1: Group-C semantics versus false completion. This is the paper's most direct
    # visualization of semantic recognition vs teleological overconfidence.
    if args.summary.exists():
        ...
        if {"ECE", "ECE_Unknown"}.issubset(summary.columns):
            ...
            x = list(range(len(summary)))
            width = 0.34
            ax.bar([i - width / 2 for i in x], summary["ECE"], width=width, label="Top-label ECE")
            ax.bar([i + width / 2 for i in x], summary["ECE_Unknown"], width=width, label="Unknown-class ECE")
            ax.set_xticks(x, model_labels(summary), rotation=15, ha="right")
    ...
    # RQ2: repeated-sampling disagreement by model.
    ...
    if args.ranking.exists():
        ...
        if not ranking.empty:
            ...
            for j, signal in enumerate(cols):
                ax.bar([i + (j - (len(cols)-1)/2)*width for i in x], pivot[signal], width=width, label=signal)
            ax.axhline(0.5, linewidth=0.9, linestyle="--", label="random ranking")
            ax.set_xticks(x, [clean_model(i) for i in pivot.index], rotation=15, ha="right")
            ax.set_ylim(0, 1)
            ax.set_ylabel("Error-detection AUROC")
            ax.set_title("RQ2: Does sampling uncertainty rank failures?")
            ax.legend(fontsize=7, frameon=False, ncol=2)
            save(fig, args.outdir / "rq2_error_ranking.pdf")
            plt.close(fig)

    # Paired A->C semantic update. Plot only aggregate pair rows, not subclass rows.
    if args.pairwise.exists():
        ...
        if not agg.empty:
            for pair_name, g in agg.groupby("pair"):
                ...
                ax.set_xticks(x, [clean_model(m) for m in g["model"]], rotation=15, ha="right")
                ax.set_ylabel("Mean probability update")
                target = "P(Unknown)" if pair_name == "A_to_C" else "P(True)"
                ax.set_title(f"Paired context update {pair_name.replace('_', '→')}: Δ{target}")
                save(fig, args.outdir / f"pairwise_{pair_name}.pdf")
                plt.close(fig)

    # Robustness: flip rate and distribution movement from canonical neutral prompt.
    if args.robustness.exists():
        robust = pd.read_csv(args.robustness)
        if not robust.empty:
            robust["condition"] = robust["comparison_prompt"].astype(str) + " / " + robust["comparison_label_order"].astype(str)
            ...
            for j, condition in enumerate(conditions):
                ...
                vals = [float(g.loc[m, "label_flip_rate"]) if m in g.index else float("nan") for m in models]
                ax.bar([i + (j-(len(conditions)-1)/2)*width for i in range(len(models))], vals, width=width, label=condition)
            ax.set_xticks(range(len(models)), [clean_model(m) for m in models], rotation=15, ha="right")
            ax.set_ylim(0, 1)
    ...
    # RQ3 risk-coverage: one clean panel per model, exported as vector files.
    ...
    # RQ3 control: accuracy/TBR/atelic retention against recheck rate.
    if args.recheck.exists():
        recheck = pd.read_csv(args.recheck)
        if not recheck.empty:
            for model, sub in recheck.groupby("model"):
                sel = sub[sub["policy"] == "selective_recheck"].sort_values("recheck_rate")
                if sel.empty:
                    continue
                fig, ax = plt.subplots(figsize=(6.0, 3.5))
                ax.plot(sel["recheck_rate"], sel["risk"], marker="o", label="overall risk")
                ax.plot(sel["recheck_rate"], sel["TBR_C"], marker="s", label="TBR-C")
                ax.plot(sel["recheck_rate"], 1 - sel["group_D_accuracy"], marker="^", label="1 − Group-D accuracy")
                ax.set_xlim(left=0)
                ax.set_ylim(bottom=0)
                ax.set_xlabel("Recheck rate")
