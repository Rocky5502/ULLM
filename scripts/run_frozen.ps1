...
Write-Host "[1/7] Validate environment, preregistration, and dataset"
python scripts/preflight.py

Write-Host "[2/7] Snapshot live gateway model catalogue"
python scripts/check_models.py

$env:PYTHONPATH = "src"

Write-Host "[3/7] Balanced smoke test: 20 examples, all five models, neutral prompt"
...
Write-Host "[4/7] Full deterministic primary experiment: 400 items x 5 models"
python -m ullm.run --mode deterministic --prompt neutral --run-id frozen-det-neutral-v1

Write-Host "[5/7] Full repeated sampling: K=5 from config"
python -m ullm.run --mode sampling --prompt neutral --run-id frozen-sampling-neutral-v1

Write-Host "[6/7] Fixed 120-item prompt robustness: strict logic + definition aware"
...
Write-Host "[7/7] Fixed label-order audit on the same 120-item neutral subset"
