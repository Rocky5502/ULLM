...
echo "[1/14] Hard audit gates against frozen manifests"
...
echo "[4/14] RQ2 repeated-sampling uncertainty"
...
echo "[5/14] Unified RQ2 failure ranking"
...
echo "[6/14] RQ2 verb-cluster ranking intervals"
...
echo "[7/14] Paired A-C / B-D semantic updates"
...
echo "[8/14] Prompt and label-order robustness"
...
echo "[9/14] RQ3 threshold-realizable risk-coverage"
...
echo "[10/14] RQ3 cached selective-verifier policies"
...
echo "[11/14] Predeclared H1-H4 evidence synthesis and Holm family"
python scripts/analyze_hypotheses.py --det "${det[@]}" --bootstrap results/processed/summary_bootstrap.csv --ranking-bootstrap results/processed/uncertainty_ranking_bootstrap.csv --recheck results/processed/recheck.csv --permutations 10000 --out results/processed/hypothesis_evidence.csv

echo "[12/14] Publication vector figures"
...
echo "[13/14] Auto-generate LaTeX result tables"
...
echo "[14/14] Freeze dual-SHA analysis provenance"
python scripts/freeze_analysis_provenance.py --out results/processed/analysis_manifest.json

echo "Analysis complete. Empirical outputs are PASS-audited, preregistered evidence is synthesized without best-result selection, and derived artifacts are sealed to raw-run + analysis-code provenance."
