...
def audit_file(
    ...
    parse_errors = [
        r for r in rows if not r.get("request_error") and r.get("prediction") is None
    ]
    argmax_bad = [
        r
        for r in rows
        if r.get("prediction")
        and not r["prediction"].get("argmax_consistent", True)
    ]
    norm_bad = [
        r
        for r in rows
    ...
    returned = sorted(
        {str(r.get("model_returned")) for r in rows if r.get("model_returned")}
    )
    prompt_types = sorted({str(r.get("prompt_type", "legacy")) for r in rows})
    prompt_hashes = sorted(
        {str(r.get("prompt_sha256")) for r in rows if r.get("prompt_sha256")}
    )
    label_orders = {tuple(r.get("label_order", [])) for r in rows}
    max_tokens_values = {
        int(r["max_tokens_requested"])
        for r in rows
        if r.get("max_tokens_requested") is not None
    }
    temperatures = {
    ...
    if expected_k is not None:
        ...
        incomplete_ids = sorted(
            k for k, n in counts_by_id.items() if n != expected_k
        )
        bad_repeat_ids = sorted(
            k for k, reps in repeats_by_id.items() if reps != expected_repeats
        )
    ...
    if len(prompt_types) != 1 or len(prompt_hashes) != 1:
        failures.append(
            f"mixed/missing prompt condition hashes: types={prompt_types}, hashes={prompt_hashes}"
        )
    ...
    if incomplete_ids:
        failures.append(
            f"items not having expected K={expected_k}: {incomplete_ids[:10]} "
            f"({len(incomplete_ids)} total)"
        )
    if bad_repeat_ids:
        failures.append(
            f"items with wrong repeat index set for K={expected_k}: {bad_repeat_ids[:10]} "
            f"({len(bad_repeat_ids)} total)"
        )
    ...
    if manifest is not None:
        ...
        if missing_ids:
            failures.append(
                f"IDs missing vs manifest: {missing_ids[:10]} ({len(missing_ids)} total)"
            )
        if extra_ids:
            failures.append(
                f"IDs not present in manifest: {extra_ids[:10]} ({len(extra_ids)} total)"
            )
        expected_rows = int(manifest.get("selected_n", len(expected_ids))) * int(
            expected_k or 1
        )
        if len(rows) != expected_rows:
            failures.append(
                f"row count {len(rows)} != manifest expectation {expected_rows}"
            )
        if requested and requested[0] not in set(manifest.get("models", [])):
            failures.append(
                f"requested model {requested[0]} not in manifest models {manifest.get('models')}"
            )
        if prompt_types and prompt_types[0] != manifest.get("prompt_type"):
            failures.append(
                f"prompt_type {prompt_types[0]} != manifest {manifest.get('prompt_type')}"
            )
        ...
        if max_tokens_values and next(iter(max_tokens_values)) != int(
            manifest.get("max_tokens", -1)
        ):
    ...
    if rows and latency_present < len(rows):
        warnings.append(
            f"latency missing on {len(rows) - latency_present}/{len(rows)} rows"
        )
...
def main() -> None:
    ...
    report = [audit_file(path, args.expected_k, manifest) for path in args.paths]
    requested_files = [
    ...
    if manifest is not None:
        ...
        duplicate_model_files = [
            m for m, n in Counter(requested_files).items() if n > 1
        ]
        if missing_model_files:
            global_failures.append(
                f"missing output file(s) for manifest model(s): {missing_model_files}"
            )
        if duplicate_model_files:
            global_failures.append(
                f"multiple output files for model(s): {duplicate_model_files}"
            )
    ...
    payload = {
        ...
        "files": report,
        "status": "PASS"
        if not global_failures and all(r["status"] == "PASS" for r in report)
        else "FAIL",
    }
