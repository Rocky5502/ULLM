...
def at_coverage(records: list[dict], uncertainty: list[float], target: float) -> dict[str, float]:
    ...
    return {
        "coverage": k / len(records),
        "risk": sum(err) / len(err),
        "TBR_C": tbr,
...
def coverage_at_risk(records: list[dict], uncertainty: list[float], target_risk: float) -> float:
    order = sorted(range(len(records)), key=lambda i: uncertainty[i])
    best = 0
    errors = 0
    for k, idx in enumerate(order, start=1):
        errors += records[idx]["prediction"]["label"] != records[idx]["example"]["label"]
        risk = errors / k
        if risk <= target_risk:
            best = k
    return best / len(records) if records else float("nan")
...
def main() -> None:
    ...
    p.add_argument("--target-risks", nargs="+", type=float, default=[0.10, 0.05])
    args = p.parse_args()
    ...
    for path in args.paths:
        ...
        signals = {
            "1-maxprob": [verbal_uncertainty(r["prediction"]["probabilities"]) for r in rows],
        ...
        if sampling is not None and not sampling.empty:
            sub = sampling[(sampling.model == model) & (sampling.prompt_type == prompt_type)].set_index("id")
        ...
        for name, scores in signals.items():
            ...
            target_coverages = {f"coverage_at_risk_{r:.2f}": coverage_at_risk(rows, scores, r) for r in args.target_risks}
