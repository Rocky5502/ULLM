...
We predeclare five model families through a common OpenAI-compatible gateway: \texttt{gpt-5.6-sol}, \texttt{claude-sonnet-5}, \texttt{deepseek-v4-pro}, \texttt{qwen3.8-max}, and \texttt{gemini-3.7-flash}. These are gateway routing identifiers, not claims about immutable vendor-direct checkpoints. Immediately before the frozen run, the artifact queries the live gateway catalog and stores the catalog snapshot. Every response records both the requested and returned identifier, timestamp, decoding settings, request identifier when available, latency, and token usage. Scientific claims therefore attach to the endpoints actually observed on the run date.
...
\midrule
OpenAI & \texttt{gpt-5.6-sol} & API \\
Anthropic & \texttt{claude-sonnet-5} & API \\
DeepSeek & \texttt{deepseek-v4-pro} & API \\
Alibaba--Qwen & \texttt{qwen3.8-max} & API \\
Google & \texttt{gemini-3.7-flash} & API \\
\bottomrule
\end{tabular}
\caption{Predeclared cross-family gateway panel. Availability and returned IDs are frozen at execution time.}
\label{tab:models}
\end{table}
